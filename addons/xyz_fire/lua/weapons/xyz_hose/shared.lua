...
SWEP.ViewModel = "models/craphead_scripts/ocrp2/props_meow/weapons/c_firehose.mdl"
SWEP.WorldModel = "models/craphead_scripts/ocrp2/props_meow/weapons/w_firehose.mdl"

function SWEP:Initialize()
	self:SetHoldType("ar2") -- TODO: check if right holdtype
	...
	if SERVER then
		...
		for k, v in pairs(ents) do
			...
			if v:GetClass() == "prop_vehicle_jeep" then
				...
				v:VC_repairHealth(26)
				v.nextReduce = CurTime() + .5
			else 
				v.nextReduce = CurTime() + 1
				v:ReduceHealth(2.5, self.Owner)
			end
