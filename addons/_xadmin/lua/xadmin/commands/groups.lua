...
xAdmin.Core.RegisterCommand("setgroup", "Set a user's group", 93, function(admin, args)
	...
	if not admin:HasPower(xAdmin.Groups[args[2]].power) then
		xAdmin.Core.Msg({Color(46, 170, 200), "[xAdmin] ", Color(255, 255, 255), "You do not have enough power to set this rank"}, admin)
		return
	end
	...
	if IsValid(targetPly) and targetPly:HasPower(admin:GetGroupPower()) then
		xAdmin.Core.Msg({Color(46, 170, 200), "[xAdmin] ", Color(255, 255, 255), "This user outpowers you."}, admin)
		return
	end
	...
	if IsValid(targetPly) then
		...
		if args[2] == "jr-mod" then
			targetPly:GiveBadge("staff")
		end
	else
		xAdmin.Core.Msg({"You have updated "..target.."'s usergroup to: "..args[2]}, admin)

		if args[2] == "jr-mod" then
			XYZBadges.Core.GiveIDBadge(target, "staff")
		end
	end
end)
