	...
	if SERVER then
		if self:GetDTInt(0) == 1  then
			...
			if self.MissileType == 1 then
					...
					for k, v in pairs(ents.FindInSphere(pos, 2048)) do
						if (v:IsPlayer() and v:Alive()) or v:IsNPC() then
							...
							if DotProduct >= 0.7 and v != owner then
								...
								td.start = pos
								td.endpos = td.start + traceDir * 2048 + Vector(0, 0, 30) -- make sure we're not running the trace at the entity's feet
								td.filter = {}
				...
				if not self.TargetedEntity or self.TargetedEntity:GetClass() != "ent_ins2rpghvatrocket" then
					for k, v in pairs(ents.FindInSphere(pos, 1024)) do
						if v:GetClass() == "ent_ins2rpghvatrocket" and v != self then
							pos2 = v:GetPos()
							DotProduct = self:GetForward():DotProduct((pos2 - pos):GetNormal())
							...
							if DotProduct >= 0.6 then
								...
								td.start = pos
								td.endpos = td.start + traceDir * 1024 + Vector(0, 0, 0) -- make sure we're not running the trace at the entity's feet
								td.filter = {}
...
function ENT:OnRemove()
	...
	util.ScreenShake( self:GetPos(), 170, 50, 1.75, 1000 )
end
...
function ENT:Explode()
	...
	util.BlastDamage(self, owner, pos, 150, 285)
