...
function ENT:Initialize()
	self:SetModel("models/khrcw2/ins2rpg7herocket.mdl") 
	...
	if SERVER then
		if self:GetDTInt(0) == 1  then
			...
			if self.MissileType == 1 then
				...
				if not self.TargetedEntity or self.TargetedEntity:GetClass() != "ent_ins2rpgherocket" then
					for k, v in pairs(ents.FindInSphere(pos, 1024)) do
						if v:GetClass() == "ent_ins2rpgherocket" and v != self then
							pos2 = v:GetPos()
							DotProduct = self:GetForward():DotProduct((pos2 - pos):GetNormal())
...
function ENT:OnRemove()
	...
	util.ScreenShake( self:GetPos(), 230, 170, 1.85, 2000 )
end
...
function ENT:Explode()
	...
	util.BlastDamage(self, owner, pos, 575, 135)
