...
if CLIENT then
	att.displayIcon = surface.GetTextureID("atts/att_sb_m6x")
end

function att:attachFunc()
	self:setBodygroup(2,2)	--Change BodyGroup. #1 is a set, #2 is an option. Any $model or $body lines are counted as BodyGroups as well. Both numbers use 0 and up. So if you want to change the 1st bodygroup to the 2nd option, first try 1,1.
	if self.WMEnt then
