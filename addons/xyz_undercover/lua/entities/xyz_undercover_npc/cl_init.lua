...
local addonColor = Color(105, 110, 117)
function ENT:Draw()
	self:DrawModel()
	...
	if XYZSettings.GetSetting("overhead_position", "Side") == "Top" then
		...
		cam.Start3D2D(self:GetPos() + (self:GetUp()*67), ang, 0.07)
		offset = 80
	end
		-- Base background
		...
		XYZUI.DrawText("Undercover", 45, offset + (width*0.5), 37, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	cam.End3D2D()
end
