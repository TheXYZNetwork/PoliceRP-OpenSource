...
local addonColor = XYZ_HITMAN.Config.Color
function ENT:Draw()
	self:DrawModel()
	...
	cam.Start3D2D(self:GetPos() + (self:GetUp()*30) + (self:GetForward()*11), ang, 0.07)
		offset = -width*0.5
		-- Base background
		...
		XYZUI.DrawText("Hitman Phone", 45, offset + (width*0.5), 37, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	cam.End3D2D()
end
