...
local addonColor = G4S.Config.Color
...
function ENT:Draw()
	...
	if XYZSettings.GetSetting("overhead_position", "Side") == "Top" then
		...
		cam.Start3D2D(self:GetPos() + (self:GetUp()*19), ang, 0.07)
		offset = 60
	end
		-- Base background
		...
		XYZUI.DrawText("G6 Deposit Point", 45, offset + (width*0.5), 37, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	cam.End3D2D()
end
