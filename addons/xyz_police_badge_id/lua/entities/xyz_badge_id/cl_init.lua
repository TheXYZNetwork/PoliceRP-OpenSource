...
function ENT:Draw()
	...
	if XYZSettings.GetSetting("overhead_position", "Side") == "Top" then
		...
		draw.RoundedBox(0, offset+5, 5, width-10, 10, GovBadgeID.Config.Color)
		draw.RoundedBox(0, offset+5, 10, width-10, 5, shader)

		XYZUI.DrawText("Government Badge ID", 45, offset + (width*0.5), 37, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	cam.End3D2D()
end
