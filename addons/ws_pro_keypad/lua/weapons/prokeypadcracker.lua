...
local keypad_crack_time2 = CreateConVar("keypad_crack_time2", "10", {FCVAR_REPLICATED, FCVAR_ARCHIVE}, "Seconds for keypad cracker to crack keypad")
...
if CLIENT then
	SWEP.PrintName = "Pro Keypad Cracker"
	SWEP.Slot = 4
	SWEP.SlotPos = 1
	SWEP.DrawAmmo = false
...
SWEP.Author = "Willox, modified by Rone"
...
function SWEP:Initialize()
	...
	self.KeyCrackTime = keypad_crack_time2:GetInt()
end	
		...
		if SERVER then
			...
			timer.Create("KeyCrackSounds: "..self:EntIndex(), 1, self.KeyCrackTime, function()
				if IsValid(self) and self.IsCracking then
					self:EmitSound(self.KeyCrackSound, 100, 200)
			...
			timer.Create("KeyCrackDots: "..entindex, 1, 0, function()
