...
function ENT:Draw()
	...
	cam.Start3D2D(self:GetPos() + (self:GetUp()*60), ang, 0.07)
		...
		XYZUI.DrawText("Oven", 45, offset + (width*0.5), 37, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		XYZUI.DrawText(self:GetIsActive() and "Currently Cooking" or "Waiting", 35, offset + (width*0.5), 55, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
	cam.End3D2D()
end