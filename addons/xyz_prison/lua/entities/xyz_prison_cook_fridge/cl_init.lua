...
function ENT:Draw()
	...
	cam.Start3D2D(self:GetPos() + (self:GetUp()*90) + (self:GetForward()*28), ang, 0.07)
		...
		XYZUI.DrawText("Fridge", 45, offset + (width*0.5), 37, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		XYZUI.DrawText("Stock: "..self:GetStock(), 35, offset + (width*0.5), 55, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
	cam.End3D2D()
end