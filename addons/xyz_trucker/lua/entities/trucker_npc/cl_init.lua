...
local addonColor = Color(100, 100, 160)
function ENT:Draw()
	self:DrawModel()
	...
	if XYZSettings.GetSetting("overhead_position", "Side") == "Top" then
		cam.Start3D2D(self:GetPos() + (self:GetUp()*77.5), ang, 0.07)
		offset = -width*0.5
	else
		...
		XYZUI.DrawText("Trucking Agency", 45, offset + (width*0.5), 37, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	cam.End3D2D()
end
